...
class SingleT2FLS_Mamdani(tf.keras.Model):
    ...
    def _initialize_weight(self,InitialSetup_List):
        ...
        c1 = tf.Variable(tf.abs(tf.random.get_global_generator().normal(shape=(self.Rule_num,)))+1)     #初始化c1
    ...
    #def compute_FLSoutput(u,x):
    #    return tf.reduce_sum(tf.multiply(u,x))/tf.reduce_sum(x)
    ...
    def call(self,input_data):
        ...
        for i in range(samples_num):
            ...
            Output_Left[i] = tf.reduce_sum(tf.multiply(tf.cast(self.c1,dtype=tf.float32),tf.cast(L_U_sort,dtype=tf.float32)))/tf.reduce_sum(tf.cast(L_U_sort,dtype=tf.float32))
            Output_Right[i] = tf.reduce_sum(tf.multiply(tf.cast(self.c2,dtype=tf.float32),tf.cast(R_L_sort,dtype=tf.float32)))/tf.reduce_sum(tf.cast(R_L_sort,dtype=tf.float32))
